...
export default function AISemanticSearch({ onClose }: Props) {
  ...
  const handleSearch = async () => {
    ...
    const watchList = watches.map(w =>
      `ID:${w.slug || w.id} | ${w.brand} ${w.name} | $${w.price} | ${w.category} | ${w.rating}/5 | Water: ${w.specs?.['Water Resistance'] ?? w.specifications?.['Water Resistance'] ?? 'N/A'} | ${w.shortDescription}`
    ).join('\n');
  ...
  const extractSlugs = (text: string): string[] => {
    ...
    watches.forEach(w => {
      const wSlug = w.slug || w.id;
      if (text.toLowerCase().includes(wSlug) || text.toLowerCase().includes(w.name.toLowerCase())) {
        if (!slugs.includes(wSlug)) slugs.push(wSlug);
      }
    });
    return slugs;
  };
      ...
      {results && extractSlugs(results).length > 0 && (
        <div className="flex flex-wrap gap-2">
          {extractSlugs(results).map(slug => {
            const w = watches.find(w => (w.slug || w.id) === slug);
            if (!w) return null;
            return (
              <button
                key={slug}
